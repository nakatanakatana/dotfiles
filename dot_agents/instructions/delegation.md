...
- Include the explicit model in the delegation prompt only when one was selected; otherwise state that the runtime default model is being used. Follow `$HOME/.agents/TOOLING.md` for agent-specific model discovery and selection when required.
